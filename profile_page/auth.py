...
from profile_page.models import User, Schedule
from profile_page.utils import user as current_user
from profile_page.utils.user import get_current_user
from profile_page.utils.helpers import redirect_url, get_errors, create_session, clear_session
...
def user_info():
    ...
    if current_user.authed():
        ...
        if sch:
            return render_template('user_info.html', username=user.username, user_role=user.role, first_name = user.first_name, last_name = user.last_name, patronymic = user.patronymic, addmission_year = user.addmission_year, direction = user.direction, group_num = user.group_num, schedule = sch.schedule)
        else:
